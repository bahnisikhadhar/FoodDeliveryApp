...
function Body() {
  ...
  const [restoListUrl, setRestoListUrl] = useState(`https://fooddeliveryapp-server-nftc.onrender.com/api/restaurants?lat=${coordinates.lat}&lng=${coordinates.lng}&page_type=DESKTOP_WEB_LISTING`);
...
// console.log(coordinates)
  ...
  useEffect(() => {
    setRestaurantList([]);
    setOffset(15); 
    setRestoListUrl(`https://fooddeliveryapp-server-nftc.onrender.com/api/restaurants?lat=${coordinates.lat}&lng=${coordinates.lng}&offset=15&sortBy=RELEVANCE&pageType=SEE_ALL&page_type=DESKTOP_SEE_ALL_LISTING`);
